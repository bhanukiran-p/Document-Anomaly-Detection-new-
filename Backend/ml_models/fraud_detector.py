...
class MoneyOrderFraudDetector:
    ...
    def _mock_predict(self, features: list, feature_names: list, extracted_data: Dict) -> Dict:
        ...
        rules = [
            (feature_dict.get('issuer_valid', 0) < 1.0, 0.25),   # Invalid issuer
            (feature_dict.get('serial_format_valid', 0) == 0, 0.20),  # Serial issues
            (feature_dict.get('amount_consistent', 1.0) < 1.0, 0.15),  # Amount mismatch
            (feature_dict.get('date_is_future', 0) == 1.0, 0.40),  # Future date
            (feature_dict.get('date_age_days', 0) > 90, 0.10),  # Very old
            (feature_dict.get('amount_category', 0) >= 3.0, 0.10),  # Large amount
            (feature_dict.get('text_quality_score', 1.0) < 0.5, 0.15),  # Poor text
            (feature_dict.get('signature_present', 0) == 0, 0.10),  # Missing signature
        ]
    ...
    def _apply_strict_validation(self, base_score: float, features: list,
        ...
        # High priority rules (+0.20 to +0.30)
        ...
        # Medium priority rules (+0.10 to +0.15)
        apply(
            feature_dict.get('date_format_consistency', 1.0) == 0.0,
            0.15,
            "MEDIUM: Invalid date format (+0.15)"
        )
        apply(
            feature_dict.get('suspicious_amount_pattern', 0.0) == 1.0 and not is_paystub,
            0.10,
            "MEDIUM: Suspicious amount pattern (+0.10)"
        )
        apply(
        ...
        apply(
            feature_dict.get('serial_format_valid', 1.0) == 0.0,
            0.15,
            "MEDIUM: Invalid serial format (+0.15)"
        )

        # Low priority rules (+0.10)
        apply(
            feature_dict.get('field_quality_score', 1.0) < 0.5,
            0.10,
            "LOW: Poor field quality (+0.10)"
        )
        apply(
            feature_dict.get('signature_required_score', 0.5) == 0.0,
            0.10,
            "LOW: Missing required signature (+0.10)"
        )
